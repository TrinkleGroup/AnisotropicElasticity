/*
  ...
  Purpose: Read in two XYZ files and make the differential displacement
           map, for edge components.  Output to a fig file for plotting.
  ...
  Param.:  <perfect-xtal.file> <dislocated-xtal.file> <Rcut> <Rmax>
	   ...
	   -e    point in the direction of the edge vector
...
const int NFLAGS = 4;
const char USERFLAGLIST[NFLAGS] = {'n', 'b', 'a', 'e'};
...
int main ( int argc, char **argv ) 
{
  ...
  int NUMBERS = FLAGON[0];
  int TRIADS = FLAGON[1];
  int ATOMNUMS = FLAGON[2];
  int EDGEVEC = FLAGON[3];
  ...
  double burgers;
  fgets(dump, sizeof(dump), disloc_file);
  sscanf(dump, "%lf %lf", &ztest, &burgers);
  if (Natoms != i) {
  ...
  if ( dcomp(burgers, 0.) || (burgers < 0.) ) {
    fprintf(stderr, "burgers = %.5lf is too small\n", burgers);
    fprintf(stderr, "check to make sure that second line of XYZ file has the burgers vector.\n");
    ERROR = ERROR_BADFILE;
  }
  ...
  Natoms = 0;
  for (i=0; i<MEMORY; ++i) {
    if ( (pos_d[i][0]*pos_d[i][0]+pos_d[i][1]*pos_d[i][1]) <= Rmax2 ) {
      // Keep this atom!
      if (i == Natoms)
  ...
  for (n=0; n<3; ++n) disp[n] = new double[Natoms];
  for (i=0; i<Natoms; ++i)
    for (n=0; n<3; ++n)
      disp[n][i] = pos_d[i][n] - pos[i][n];
  ...
  auto_scale(Natoms, pos_d, a0, x0, y0, r_atom, portrait);
  ...
  for (i=0; i<Natoms; ++i) {
    ...
    draw.circle(pos_d[i][0], pos_d[i][1], r_atom);
    if(ATOMNUMS) {
      sprintf(dump, "%d", i+1);
      draw.text(pos_d[i][0], pos_d[i][1]-r_atom, dump);
    }
  }
  ...
  // burgers = | 1/12[112] | / | 1/4[110] |  <- note bottom
  //double burgers = sqrt(1./3.) * z_thick;
  //burgers vector length is defined in the xyz file

  // Replace the "dscale" with the nn. dist scale (i.e., 1/3 b == nn dist)
  // Set text size:
  double dscale = 1.; // Scale factor... not sure what to do here.
  ...
  for (i=0; i<NNpairs; ++i) {
    ...
    if (nn_pair->j > nn_pair->i) {
      for (n=0; n<3; ++n)
	dd_vect[n] = disp[n][nn_pair->j] - disp[n][nn_pair->i];
      ...
      */
      ...
      if(!EDGEVEC) {
	      ...
	      double blen = dd_vect[0];

	      while (blen > 0.5*burgers) blen -= burgers;
	      while (blen <-0.5*burgers) blen += burgers;
	      ...
	      dd_len = fabs(blen) / burgers;
      } else {
	      vx = dd_vect[0];
	      vy = dd_vect[1];
	      double blen = sqrt(vx*vx + vy*vy);
	      while(blen > 0.5*burgers) {
		      vx *= 1.0 - burgers/blen;
		      vy *= 1.0 - burgers/blen;
		      blen = sqrt(vx*vx + vy*vy);
	      }
	      dd_len = sqrt(vx*vx + vy*vy) / burgers;
      }


      // dscale determines what length of b is equal to the nn dist:
      draw.cvector(x, y, vx*dscale, vy*dscale);
      if (NUMBERS) {
	// Output a number there too.
	if (dd_len >= 0.01) {
	  sprintf(dump, "%.0lf%%", dd_len*100.);
	  draw.text(x, y, dump);
	}
      }
    }
